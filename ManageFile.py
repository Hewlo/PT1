...
resource_directory = os.getcwd() + '/resource'
txt_file_directory = os.getcwd() + '/resource/student_submission.txt'
...
with open(txt_file_directory, 'r') as txt_contents:
    for lines in txt_contents.readlines():
        ...
        target_project_name = resource_directory + '/' + project
        try:
            os.rename(target_project_name, resource_directory + '/' + project_name)
...
with zipfile.ZipFile(zip_file,'w') as archive:
    for files in os.listdir(resource_directory):
        file_path = backup_folder_directory + '/' + files
